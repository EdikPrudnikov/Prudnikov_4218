<template>   
  ...
  <div class="container">   
    <h1>Страница "Презентации 3-4 года"</h1>   
...
<script>  
export default {  
  name: 'GoToButtonPPT3_4',  
  data() {  
    return {  
      searchQuery: '',  
      items: [  
        { id: 1, name: 'незнайка пдд' },  
  ...
  methods: {  
    openLink(item, action) {  
      const links = {  
        'незнайка пдд': { скачать: 'https://getfile.dokpub.com/yandex/get/https://disk.yandex.ru/i/7F8iarCnHi8rog' },  
    ...
    getImages(item) { 
      const images = { 
        'незнайка пдд': ['src/assets/незнайка пдд/незнайка пдд (2).png','src/assets/незнайка пдд/незнайка пдд (3).png','src/assets/незнайка пдд/незнайка пдд (4).png','src/assets/незнайка пдд/незнайка пдд (1).png'], 
